#!/usr/bin/env node
// Requirement-ID guard (PowerFlow D1, opt-in — extras/guards). A requirement ID
// `SRS-<FEAT>-<NNN>` is DEFINED where it appears as the first cell of a table
// row in docs/srs/*.md; references elsewhere are fine. Rules enforced:
//   1. an ID is defined exactly once across docs/srs/
//   2. a tag (FEAT) is defined in exactly one file
//   3. when docs/srs/README.md has a tag table, every defined tag is listed
//      against the same file (and below "Next free" when that column exists)
...
// --- tag table in README.md ---------------------------------------------------
// Lean form (D15): `| Tag | File | Feature |` — tag ownership only.
// Full form: `| Tag | File | Feature | Allocated | Next free |` — also checks that
// no defined number has reached "Next free". Either is accepted; no table = skip.
const registry = new Map() // tag -> { file, nextFree? }
let hasTable = false
try {
  const readme = readFileSync(join(srsDir, "README.md"), "utf8").split(/\r?\n/)
  for (const line of readme) {
    if (/^\|\s*Tag\s*\|\s*File\s*\|/i.test(line)) hasTable = true
    const m = /^\|\s*([A-Z][A-Z0-9]*)\s*\|\s*\[?([^\]|]*?)\]?(?:\([^)]*\))?\s*\|(.*)$/.exec(line)
    if (!m) continue
    const rest = m[3].split("|").map((c) => c.trim()).filter(Boolean)
    const last = rest[rest.length - 1]
    const nextFree = rest.length >= 3 && /^\d{3,}$/.test(last) ? Number(last) : undefined
    registry.set(m[1], { file: m[2].trim().replace(/^`|`$/g, ""), nextFree })
  }
} catch {
  fail("docs/srs/README.md is missing — it lists which file owns each tag")
}
if (hasTable) {
  for (const [tag, set] of tagFiles) {
    const file = [...set][0]
    const reg = registry.get(tag)
    if (!reg) { fail(`tag ${tag} (in ${file}) is not listed in docs/srs/README.md's tag table`); continue }
    if (reg.file && reg.file !== file) fail(`tag ${tag} is listed for ${reg.file} but defined in ${file}`)
    const max = tagMax.get(tag)
    if (reg.nextFree !== undefined && max >= reg.nextFree) fail(`tag ${tag}: SRS-${tag}-${String(max).padStart(3, "0")} is defined but README says next free is ${String(reg.nextFree).padStart(3, "0")}`)
  }
}
