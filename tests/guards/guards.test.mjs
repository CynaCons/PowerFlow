// Tests for the three vendored guards (D13). Each case builds a minimal repo in
// a temp dir and runs the script from templates/scripts against it.
...
const scripts = join(pack, "templates", "scripts")
...
for (const [name, files, expect] of [
  ...
  }, /registered to SRS-nav.md but defined in SRS-navrail.md/],
...
function versionScript(dir, source, derived = "") {
  // render the template the way stamp.py does, plus a DERIVED entry when asked
  let s = readFileSync(join(scripts, "check-version.mjs"), "utf8").replace("{{version_source}}", source)
